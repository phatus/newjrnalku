...
export default function Sidebar({ user, profile }: SidebarProps) {
    ...
    if (!user) return (
        <>
            <style dangerouslySetInnerHTML={{ __html: "@media print { aside { display: none !important; width: 0 !important; height: 0 !important; } }" }} />
            <aside className="fixed left-0 top-0 bottom-0 z-40 hidden w-72 flex-col border-r border-slate-200 bg-white sm:flex items-center justify-center p-8">
                <Link href="/login" className="px-6 py-2 bg-amber-500 text-white rounded-xl font-bold">Masuk</Link>
            </aside>
        </>
    );
